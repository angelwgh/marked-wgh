...
(function($, document) {
	...
	var ListPicker = $.ListPicker = $.Class.extend({
		...
		_scrollEndHandle: function() {
			...
			var point=(scrollTop / self.itemHeight)-parseInt(scrollTop / self.itemHeight);
		...
		refreshPicker: function (flag) {
	        var self = this;
	        if(!self.box.isEye)return;
	        if(self.options.layer<4)return;
	        var width = (100 / self.options.layer);
	        if (flag) {
	            width = (100 / (self.options.layer + 1));  
	            self.box.style.width = width * 2 + '%';
	        } else {
	            self.box.style.width = width + '%';
	        }
	        var temp = self.box.nextElementSibling;
	        while (true) {
	            if (!temp) break;
	            temp.style.width = width + '%';
	            temp = temp.nextElementSibling;
	        }
	        temp = self.box.previousElementSibling;
	        while (true) {
	            if (!temp) break;
	            temp.style.width = width + '%';
